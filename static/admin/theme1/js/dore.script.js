...
$.dore = function (element, options) {
  ...
  function init() {
      ...
      if (document.getElementById("polarChart")) {
        ...
        var myChart = new Chart(polarChart, {
          ...
          data: {
            ...
            labels: ["Claimed", "Accepted", "Rejected"]
          }
        });
      }
    ...
    if ($().DataTable) {
      ...
      var $featureDataTable = $(".data-table-feature").DataTable({
      ...
      if($.trim( $('#customerSearchField').val() ).length > 0) {
        $featureDataTable.search($('#customerSearchField').val()).draw();
      }
      // Datatable with rows
    ...
    if ($().smartWizard) {
      ...
      $("#smartWizardCustomButtons .next-btn").on("click", function (e) {
       if(".notvalidate"){
      ...
      $("#smartWizardValidation .next-btn").on("click", function () {
        if($("#cpatchaTextBox").is(':visible')){
          var res=validateCaptcha();  
          if(res==false)
          {
              return false;
          }
