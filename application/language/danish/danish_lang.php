<?php
$lang['text_home'] = 'HOME';
$lang['text_about_us'] = 'ABOUT US';
$lang['text_faqs'] = 'FAQS';
$lang['text_contact_us'] = 'CONTACT US';
$lang['text_enter_postcode'] = 'Enter Postcode';
$lang['text_search'] = 'Search';
$lang['text_title'] = 'Title';
$lang['text_quick_links'] = 'Quick Links';
$lang['text_job'] = 'Job';
$lang['text_login'] = 'Login';
$lang['text_profile'] = 'Profile';
$lang['text_logout'] = 'Logout';
$lang['text_signup'] = 'Signup';
$lang['text_my_account'] = 'My Account';
$lang['text_mobile_apps'] = 'Mobile Apps';
$lang['text_mobile'] = 'Mobil';
$lang['text_address'] = 'ADDRESS';
$lang['text_telephone'] = 'Telephone';
$lang['text_subscribe_us'] = 'SUBCRIBE US';
$lang['text_subscribe'] = 'Subscribe';
$lang['text_email_address'] = 'E-MAIL address';
$lang['text_email'] = 'E-mail';
$lang['text_get_over_weekly_discount'] = 'Get over weekly discount';
$lang['text_more_info'] = 'More Info';
$lang['text_your_selected_station'] = 'YOUR SELECTED STATION';
$lang['text_change_station'] = 'CHANGE STATION';
$lang['text_price'] = 'PRICE';
$lang['text_action'] = 'ACTION';
$lang['text_book_now'] = 'BOOK NOW';
$lang['text_small'] = 'Small';
$lang['text_medium'] = 'Medium';
$lang['text_large'] = 'Large';
$lang['text_select_package'] = 'SELECT PACKAGE';
$lang['text_selected_packages'] = 'SELECTED PACKAGES';
$lang['text_registration'] = 'REGISTRATION';
$lang['text_packages'] = 'Packages';
$lang['text_select_time_slot'] = 'Select Time Slot';
$lang['text_cart'] = 'Cart';
$lang['text_package'] = 'Pacakge';
$lang['text_date'] = 'Date';
$lang['text_timeslot'] = 'Timeslot';
$lang['text_price_small_words'] = 'Price';
$lang['text_action_small_words'] = 'Action';
$lang['text_checkout'] = 'CHECK OUT';
$lang['text_addmore'] = 'ADD MORE';
$lang['text_name'] = 'Name';
$lang['text_email'] = 'E-mail';
$lang['text_telephone'] = 'Telephone';
$lang['text_subject'] = 'Subject';
$lang['text_message'] = 'Message';
$lang['text_send'] = 'Send';
$lang['text_reg_nr'] = 'Reg Nr.';
$lang['text_comments'] = 'Comments';
$lang['text_terms_conditions'] = 'Terms & Conditions';
$lang['text_book_now'] = 'Book Now';
$lang['text_pay_now'] = 'Pay Now';
$lang['text_print_invoice'] = 'Print Invoice';
$lang['text_invoice_number'] = 'Invoice No.';
$lang['text_booking_successfull'] = 'Booking Successfull';
$lang['text_total'] = 'Total';
$lang['text_address_small_words'] = 'Address';
$lang['text_company'] = 'Company (optional)';
$lang['text_ETG'] = 'ETG (optional)';
$lang['text_OppgangEtasje'] = 'Increase (optional)';
$lang['text_payment_successfull'] = 'Payment Successful';
$lang['text_your_cart_is_empty'] = 'Your cart is empty';
$lang['text_add_to_cart_msg'] = 'The product has been added to your cart!';
$lang['text_cart_item_delete_msg'] = 'The product has been deleted from your cart successfully!';
$lang['text_cart_has_been_emptied'] = 'The cart has been emptied!';
$lang['text_select_station'] = 'Select station!';
$lang['text_payment_successfull_for_following_booking'] = 'Payment Successful for following booking';
$lang['text_lets_deal_code'] = 'Lets Deal Code';
$lang['text_noti_confirm_password'] = 'Please re enter confirm password';
$lang['text_noti_update_password'] = 'Password updated successfully.';
$lang['text_noti_same_email_exist'] = 'A customer with same email already exists.';
$lang['text_noti_profile_updated_successfully'] = 'Profile updated successfully.';
$lang['text_noti_please_enter_name'] = 'Please enter name.';
$lang['text_noti_please_enter_Email'] = 'Please enter Email.';
$lang['text_noti_please_enter_phone_number'] = 'Please enter phone number.';
$lang['text_noti_please_enter_mobile_number'] = 'Please enter mobile number.';
$lang['text_noti_please_check_terms_conditions_checkbox'] = 'Please check terms & conditions checkbox.';
$lang['text_noti_please_enter_address'] = 'Please enter address';
$lang['text_noti_please_enter_post_code'] = 'Please enter post code.';
$lang['text_noti_please_enter_city'] = 'Please enter city.';
$lang['text_login_successful'] = 'Login successful.';
$lang['text_logout_successful'] = 'Logout successful.';
$lang['text_user_not_found'] = 'User not found.';
$lang['text_search_item'] = 'Search Item';
$lang['text_min'] = 'Min';
$lang['text_max'] = 'Max';
$lang['text_add_to_order'] = 'Add to order';
$lang['text_your_order'] = 'Your Order';
$lang['text_your_orders'] = 'Orders';
$lang['text_order_no'] = 'Order';
$lang['text_order_booking_msg'] = 'Thank you for your order.';
$lang['text_order_booking_msg_2'] = 'Thank you for your order (Order #: ';
$lang['text_delivery'] = 'Delivery';
$lang['text_take_away'] = 'Take Away';
$lang['text_eat_here'] = 'Eat Here';
$lang['text_quantity'] = 'Quantity';
$lang['text_product'] = 'Product';
$lang['text_action'] = 'Action';
$lang['text_item_charges_tax_excl'] = 'Item Charges (Tax Exclusive)';
$lang['text_discount'] = 'Discount';
$lang['text_amount'] = 'Amount';
$lang['text_doorbell_name'] = 'Alarm Clock Name';
$lang['text_item_charges'] = 'Item Charges';
$lang['text_vat'] = 'VAT';
$lang['text_vat_included'] = 'VAT Included';
$lang['text_delivery_charges'] = 'Delivery Charges';
$lang['text_free_delivery'] = 'Gratis';
$lang['text_delivery_charges_vat'] = 'Delivery Charges VAT';
$lang['text_total_charges'] = 'Total Charges';
$lang['text_total_cost'] = 'Total Cost';
$lang['text_remove_all_items'] = 'Remove all items';
$lang['text_checkout'] = 'Checkout';
$lang['text_your_cart_is_empty'] = 'Your cart is empty.';
$lang['text_what_would_you_like'] = 'What would you like to order today?';
$lang['text_already_registered_login_here'] = 'If you are already registered login here';
$lang['text_password'] = 'Password';
$lang['text_username'] = 'Username';
$lang['text_forgot_password'] = 'Forgot Password?';
$lang['text_register'] = 'Register';
$lang['text_enter_captcha_code'] = 'Enter Captcha Code';
$lang['text_already_registered_enter_email_here'] = 'If you are already registered enter your e-mail here to reset your password.';
$lang['text_reset'] = 'Reset';
$lang['text_change_your_password'] = 'Change Your Password ';
$lang['text_current_password'] = 'Current Password';
$lang['text_new_password'] = 'New Password';
$lang['text_confirm_new_password'] = 'Confirm New Password';
$lang['text_change_password'] = 'Change Password';
$lang['text_password_updated_successfully'] = 'Password updated successfully.';
$lang['text_delivery_note'] = 'Delivery Note';
$lang['text_email_not_exist'] = 'Email does not exsist.';
$lang['text_password_mismatch'] = 'Passwod Mismatch.';
$lang['text_email_sent'] = 'Email sent.';
$lang['text_registration_failed'] = 'Registration Failed.';
$lang['text_wrong_captcha_code'] = 'Wrong Captcha Code.';
$lang['text_customer_with_same_username_exists'] = 'A customer with same username already exists.';
$lang['text_customer_with_same_email_exists'] = 'A customer with same email already exists.';
$lang['text_registration_successful'] = 'Registration Successful.';
$lang['text_login_to_proceed'] = 'Login to proceed.';
$lang['text_order_placed_successfuly'] = 'Order successfuly placed.';
$lang['text_you_have_cancelled_payment_cont_to_cart'] = 'You have cancelled payment. Continue to cart.';
$lang['text_check_your_email'] = 'Please check your email.';
$lang['text_personal_info'] = 'Personal Information';
$lang['text_delivery_info'] = 'Delivery Information';
$lang['text_update_profile'] = 'Update Profile';
$lang['text_place_order'] = 'Place Order';
$lang['text_post_code'] = 'Postcode';
$lang['text_city'] = 'City';
$lang['text_Pay_on_delivery'] = 'Pay on Delivery';
$lang['text_follow_us'] = 'Follow Us';
$lang['text_footer_address'] = 'Address';
$lang['text_company_agreement'] = 'Company Agreement';
$lang['text_company_agreement_desc'] = 'For companies we offer company deal with its own account. This saves you to pay cash on delivery.';
$lang['text_company_agreement_desc_link'] = 'Please contact us for deal';
$lang['text_catering'] = 'Catering and caterers';
$lang['text_catering_desc'] = '<p class = "footer p"> We offer catering and delivery to the company. <a href="kontakt-oss" class="footer-p-link"> Contact details for good prices and want! </ p> ';
$lang['text_promation_code'] = 'Promo Code';
$lang['text_do_you_have_pormation_code'] = 'Do you have promotional code.? Enter in the box';
$lang['text_search_for'] = 'Search ...';
$lang['text_go'] = 'Go';

$lang['text_branch_closed_msg'] = 'Branch Closed!';
$lang['text_branch_opening_hours_msg'] = 'Opening hours: ';

$lang['text_confirm_password'] = 'Confirm password';
$lang['text_name_blank'] = 'The name can not be empty';
$lang['text_email_blank'] = 'Email can not be empty';
$lang['text_password_blank'] = 'Password can not be empty';
$lang['text_confirm_password_blank'] = 'confirm password can not be empty';
$lang['text_password_match'] = 'passwords did not match';
$lang['text_select_station'] = 'Select Location';
$lang['text_footer_quick_links'] = 'Quick Links';
$lang['text_download_app'] = 'Download The App';




// ------------Header

$lang['text_hjem'] = 'HJEM';
$lang['text_segmenter'] = 'SEGMENTER';
$lang['text_kunst'] = 'KUNST';
$lang['text_energy'] = 'ENERGI';
$lang['text_sport'] = 'SPORT';
$lang['text_cyber'] = 'CYBERFORSIKRING';
$lang['text_corporate'] = 'CORPORATE CLIENTS';
$lang['text_forbund'] = 'FORBUND OG FORENINGER';
$lang['text_header_kontakt_oss'] = 'KONTAKT OS';
$lang['text_om_ags'] = 'OM AGS';
$lang['text_nyheter'] = 'NYHEDER';


// ------------Home

$lang['text_AGS'] = 'AGS FORSIKRING';
$lang['text_heading1'] = 'Coverholder for';
$lang['text_heading2'] = 'Lloyd’s Insurance Company S.A';
$lang['text_vare_segmenter'] = 'Vores segmenter';
$lang['text_type1'] = 'Kunst';
$lang['text_type2'] = 'Energi';
$lang['text_type3'] = 'Sport';
$lang['text_type4'] = 'Cyberforsikring';
$lang['text_type5'] = 'Corporate Clients';
$lang['text_type6'] = 'Forbund og Foreninger';
$lang['text_type1_desc'] = 'AGS Forsikring er en af de få aktører, der specialiserer sig i kunstforsikring for museer, kunsthaller, gallerier, private og offentlige samlinger.';
$lang['text_type2_desc'] = 'AGS Forsikring AS blev godkendt som agent i 2008 af Finanstilsynet i Norge og tegner forskellige energiforsikringer på vegne af Lloyd’s Insurance Company.';
$lang['text_type3_desc'] = 'Siden 2008 har AGS introduceret flere nye dækninger end nogen anden aktør til gavn for idrætsforbund og deres medlemmer på en omkostningseffektiv måde:';
$lang['text_type4_desc'] = 'Cybersikkerhed behøver ikke at skabe frygt – men det kræver forberedelse. Sammen med vores partnere med over 20 års erfaring og tusindvis af håndterede cyberhændelser ved vi, at virksomheder med integreret cybersikkerhed står stærkere.';
$lang['text_type5_desc'] = 'AGS Forsikring offers tailored insurance participations for corporate clients through brokers. Our focus is on providing excess capacity and supporting reputable lead insurers, particularly where captive retentions are significant.';
$lang['text_type6_desc'] = 'AGS Forsikring AS leverer skræddersyede forsikringsløsninger til forbund og foreninger. Med mange års erfaring og solide referencer giver vi tryghed gennem professionelle og tilpassede ordninger.';
$lang['text_read_more'] = 'Læs mere';

// ------------Art

$lang['text_kunst_heading'] = 'Kunstforsikring';
$lang['text_kunst_sub_heading'] = 'Vores erfaring og nærhed til kunstverdenen – din sikkerhed';
$lang['text_interest'] = 'INTERESSERET?';
$lang['text_kontakt_oss'] = 'Kontakt os:';
$lang['text_Seniorrådgiver'] = 'Seniorrådgiver, Kunstforsikring';
$lang['text_Seniorkonsulent'] = 'Senior Konsulent, Kunstforsikring';
$lang['text_SKADEMELDING'] = 'SKADEMELDING';
$lang['text_Vennligst'] = 'Udfyld venligst formularen nedenfor og send den til:';
$lang['text_last_ned'] = 'Download formular (PDF)';
$lang['text_kunst_line1'] = 'AGS Forsikring er en af de få eksperter inden for kunstforsikring for museer, kunsthaller, gallerier, private og offentlige samlinger. Vi forsikrer de fleste kunstmuseer i Norge og er godt repræsenteret i kunstinstitutioner i Danmark og Sverige.';
$lang['text_kunst_line2'] = 'Med mange års erfaring på det skandinaviske marked ved vi, hvad vores kunder har brug for. For os er "hvem" vi forsikrer lige så vigtigt som "hvad" vi forsikrer. At forstå kundernes behov og kriterier gør det muligt for os at tilbyde skræddersyede forsikringsløsninger.';

// ------------Energy

$lang['text_informasjon'] = 'Kontakt vores konsulent for mere information:';
$lang['text_energy_title'] = 'Energi – Onshore & Offshore';
$lang['text_energy_sub_heading'] = 'AGS tilbyder den bedste forsikring, så du kan føle dig tryg.';
$lang['text_energy_line1'] = 'AGS Forsikring AS blev godkendt som agent i 2008 af Finanstilsynet i Norge og Sverige og tegner forskellige energiforsikringer på vegne af Lloyds Insurance Company. Gennem vores partnere tilbyder AGS en række forsikringsløsninger til mæglere på vegne af deres kunder.';
$lang['text_energy_line2'] = 'AGS tilstræber at være åben, inkluderende og meget tilgængelig. Vi håber, at vores kombination af erfaring og regional viden vil give mæglere og deres kunder attraktive løsninger.';

// ------------Sport

$lang['text_sport_title'] = 'Sport';
$lang['text_sport_sub_heading'] = 'AGS tilbyder den bedste forsikring, så du kan føle dig tryg.';
$lang['text_sport_info'] = 'Kontakt vores konsulent for mere information';
$lang['text_sport_line1'] = 'Siden 2008 har AGS introduceret flere nye dækninger end nogen anden aktør til gavn for idrætsforbund og deres medlemmer på en omkostningseffektiv måde:';
$lang['text_sport_line2'] = '- Overbelastningsskader';
$lang['text_sport_line3'] = '- Kombinerede symptomer';
$lang['text_sport_line4'] = '- Spiseforstyrrelser';
$lang['text_sport_line5'] = '- Behandlingsgarantier';
$lang['text_sport_line6'] = 'Forskellige sportsgrene kræver skræddersyede forsikringsløsninger. SPORT SECURE er vores unikke forsikringskoncept for sport. Vi har håndteret over 25.000 skader under licensforsikring for atleter og sikrer kvalitetskontrol af vores sundhedsudbydere. Vi samarbejder bl.a. med Idrætsmedicinsk Center og Skadehotline og har afgjort over 25.000 skader med fokus på atleternes helbred.';
$lang['text_sport_line7'] = 'SPORT SECURE – den bedste service og de bedste vilkår på markedet fra et team, der giver lige stor opmærksomhed til breddeidræt og de individuelle behov hos eliteatleter. AGS Forsikring – specialistselskabet inden for sport.';
$lang['text_sports_skade']='Åbn idrætsforeningens hjemmeside og søg efter AGS Forsikring, hvorefter du finder en elektronisk formular, som du udfylder. Efter indsendelse modtager du et automatisk svar, hvorfra du kan fortsætte dialogen med den tildelte skadesbehandler.';

// ------------Team

$lang['text_team'] = 'Teammedlemmer';
$lang['text_designation1'] = 'Direktør';
$lang['text_designation2'] = 'Seniorrådgiver, Kunstforsikring';
$lang['text_designation3'] = 'Konsulent, Energi';
$lang['text_designation4'] = 'Skadebehandler';

// ------------About Us

$lang['text_about_title'] = 'Om os';
$lang['text_about_line1'] = 'AGS Forsikring AS blev etableret i 2008 og er licenseret af Finanstilsynet som forsikringsmægler i Norge, Sverige og Danmark.';
$lang['text_about_line2'] = 'AGS’s vision og forretningsidé – at være en førende virksomhed, der opfattes som en af de bedste inden for sine segmenter.';
$lang['text_about_line3'] = 'Forsikring forbindes med tryghed. Vores opgave er at forankre denne tryghed hos vores kunder. Det er et stort og vigtigt ansvar, som vi er bevidste om. Vores kunder skal være sikre på, at deres forsikring matcher de faktiske risici, at dækningen er korrekt dimensioneret, at vilkårene fremstår som "de bedste på markedet", at skader håndteres hurtigt og effektivt, og at vores rådgivning og administration har høj forsikringsfaglig ekspertise.';
$lang['text_about_line4'] = 'Vores medarbejdere har solid kompetence og erfaring inden for deres forretningsområder, og med de muligheder, vores kunder giver os, sigter vi mod at levere "markedets bedste service".';
$lang['text_about_line5'] = 'Velkommen til tryghed, kompetence og den bedste service på markedet!';
$lang['text_about_line6'] = 'AGS Forsikring AS';

// ------------Footer

$lang['text_skadesenter'] = 'Skadecenter';
$lang['text_administrasjon'] = 'Administration';
$lang['text_footer_link1'] = 'Kunst';
$lang['text_footer_link2'] = 'Sport og sportsbegivenheder';
$lang['text_footer_link3'] = 'Energi, on- og offshore';
$lang['text_footer_link4'] = 'Cyberforsikring';
$lang['text_footer_link5'] = 'Corporate Clients';
$lang['text_footer_link6'] = 'Forbund og Foreninger';
$lang['text_need_help'] = 'Brug for hjælp?';
$lang['text_contact_us'] = 'KONTAKT OS';
$lang['text_privacy_policy'] = 'PRIVATLIVSPOLITIK';

// ------------Privacy Policy

$lang['text_pp_title'] = 'Privatlivspolitik';
$lang['text_pp_sub_title'] = 'Behandling og registrering af personoplysninger';
$lang['text_pp_desc'] = '<br>I overensstemmelse med gældende love og retningslinjer indsamler og behandler vi personoplysninger, der er nødvendige for vores virksomhed. De specifikke oplysninger, vi indsamler, afhænger af karakteren af din interaktion med os. Nedenfor er en liste over de typer personoplysninger, vi typisk indsamler:
    <br>- Kontaktoplysninger
    <br>- Certifikat- og forsikringsoplysninger
    <br>- Skadesoplysninger
    <br>- Betalingsoplysninger
    <br>- Helbreds- og medicinske oplysninger*
    <br>Vi indsamler disse oplysninger for at kunne tilbyde forsikringer og administrere forsikringsprodukter. E-mailadresser og telefonnumre bruges primært til kommunikationsformål.
    <br><br><h4>Videregivelse af personoplysninger</h4>
    <br>Vi behandler dine personoplysninger med streng fortrolighed. Vi videregiver dem kun, når det er nødvendigt og i overensstemmelse med loven. AGS Insurance AS er en forsikringsmægler, og vi deler dine oplysninger med det eller de forsikringsselskaber, der er tilknyttet din forsikring, når det er nødvendigt. Du finder navnene på dine forsikringsselskaber på dit forsikringscertifikat. Derudover kan vi i tilfælde af skade dele oplysninger med taksatorer. Vi er også lovmæssigt forpligtet til at videregive oplysninger til offentlige myndigheder, når det er nødvendigt.
    <br><br><h4>Hvor længe opbevarer vi dine personoplysninger?</h4>
    <br>Vi opbevarer dine personoplysninger så længe, det er nødvendigt. Normalt betyder det, at vi beholder oplysningerne, mens din forsikring er aktiv, i overensstemmelse med lovkrav. Når oplysningerne ikke længere er nødvendige, vil de blive slettet.
    <br><br><h4>Beskyttelse af personoplysninger</h4>
    
    <br>Dine personoplysninger er beskyttet gennem hele processen. Vi har implementeret tekniske og organisatoriske foranstaltninger for at sikre dine data, herunder:
    <br>- Datakryptering under transmission og lagring
    <br>- Antivirusbeskyttelse på vores servere
    <br>- Regelmæssige backup-procedurer
    <br>- Adgangskodebeskyttede IT-systemer med bruger-specifikke legitimationsoplysninger
    <br>- VPN og kryptering til fjernarbejde og brug af mobile enheder
    <br>- Retningslinjer og procedurer for håndtering og deling af personoplysninger
    <br>- Uddannelse af medarbejdere i bedste praksis for datasikkerhed

    <br><br><h4>Databehandlere</h4>
    <br>Vi anvender tredjeparts databehandlere til tjenester som software, hosting, sikkerhed og bortskaffelse af data. Selvom disse behandlere kan håndtere personoplysninger på vores vegne, har de ikke direkte adgang til dine oplysninger. Vi har indgået kontrakter med hver IT-leverandør for at sikre, at de overholder vores strenge instruktioner om datasikkerhed. Disse kontrakter kræver, at behandlerne implementerer passende tekniske og organisatoriske foranstaltninger for at forhindre utilsigtet eller ulovlig ødelæggelse, tab eller uautoriseret videregivelse af dine data, i overensstemmelse med gældende lovgivning.

    <br><br><h4>Dine rettigheder</h4>
    <br>Som registreret har du ret til at udøve følgende rettigheder, med visse lovbestemte undtagelser:
    <br>- Ret til at blive informeret (GDPR artikel 12-14): Du har ret til at blive informeret om, hvordan dine personoplysninger indsamles og bruges.

    <br>- Ret til indsigt (GDPR artikel 15): Du kan anmode om at se og få en kopi af dine personoplysninger.

    <br>- Ret til berigtigelse (GDPR artikel 16): Du kan anmode om rettelse af unøjagtige eller forældede personoplysninger.

    <br>- Ret til at trække samtykke tilbage (GDPR artikel 7): Hvis du tidligere har givet samtykke til databehandling, kan du til enhver tid trække det tilbage uden at påvirke lovligheden af tidligere behandling. Kontakt os for flere detaljer om dit samtykke.

    <br>- Ret til sletning (GDPR artikel 17): Du kan anmode om at få slettet dine personoplysninger. Denne ret kan være underlagt undtagelser, såsom lovkrav. Vi sletter kun dine oplysninger, hvis du ikke længere er kunde, og når der ikke kan rejses krav mod os vedrørende tidligere hændelser eller forsikringer.

    <br>- Ret til dataportabilitet (GDPR artikel 20): Du kan anmode om, at dine oplysninger overføres til en anden dataansvarlig eller udleveres til dig i et maskinlæsbart format.

    <br>- Ret til begrænsning af behandling (GDPR artikel 18): Du kan anmode om at få begrænset eller stoppet behandlingen af dine personoplysninger.

    <br>- Ret til indsigelse (GDPR artikel 21): Du kan gøre indsigelse mod behandlingen af dine personoplysninger.

    <br>- Ret til at gøre indsigelse mod automatiseret behandling (GDPR artikel 22): Du har ret til at gøre indsigelse mod beslutninger, der udelukkende træffes ved hjælp af automatiserede processer eller profilering.';
    $lang['text_no_news'] = 'Ingen nyheder endnu.';

//----------- Cyber


    $lang['text_cyber_title'] = 'Cyberforsikring';
$lang['text_cyber_sub_heading'] = 'Full Spectrum Cyber – Fremtidens cybersikkerhed';
$lang['text_cyber_line1'] = 'Cybersikkerhed behøver ikke at skabe frygt – men det kræver forberedelse.<br>
Sammen med vores partnere med over 20 års erfaring og tusindvis af håndterede cyberhændelser ved vi, at virksomheder med integreret cybersikkerhed står stærkere. Derfor tilbyder vi en helhedsløsning, der styrker din evne til at forudse, håndtere og tilpasse dig digitale trusler.
<br>
Fuld dækning med Full Spectrum Cyber
<br>
Vi kombinerer det anerkendte Beazley Breach Response-produkt med ekstra sikkerhedstjenester fra Beazley Security. Resultatet? Øget robusthed gennem adgang til teknologi, træning og support – alt i én løsning.
<br>
Hvem passer det til?
<br>
Mellemstore og komplekse virksomheder i forskellige brancher over hele verden.
<br>
Hvad dækker vi?
<br>
Op til 15 millioner euro i standarddækning – mulighed for udvidelse til 25 millioner euro
<br>
Breach response: juridisk bistand, IT-forensik, varsling, kundecentre, kredit- og ID-overvågning, PR og krisehåndtering
<br>
Skræddersyet støtte i mødet med regulatoriske krav og gruppesøgsmål
<br>
Vores tredelte tilgang
<br>
<strong>1. Forebyggende tiltag</strong><br>
Vi styrker cybersikkerheden gennem:<br>
-Anti-phishing-kampagner og træning<br>
-Sikkerhedsvurdering af Microsoft 365<br>
-Ransomware-parathed<br>
-MFA-nøgler og simuleringer
<br>
<strong>2. Respons ved hændelser</strong><br>
Ved et angreb stiller vores eksperter straks op med:<br>
-Krisehåndtering og juridisk rådgivning<br>
-Forensik og gendannelse af data<br>
-Forhandling med ransomwarekriminelle<br>
-PR- og omdømmestøtte
<br>
<strong>3. Tilpasning og udvikling</strong><br>
Vi tilbyder løbende trusselsanalyse, webinarer, træning og adgang til ressourcer – så din virksomhed altid ligger et skridt foran.';


//-------------- Corporate Clients



$lang['text_corporate_title'] = 'Corporate Clients';
$lang['text_corporate_info'] = 'Kontakt vores konsulent for mere information';
$lang['text_corporate_sub_heading'] = 'AGS Forsikring ? Corporate Insurance Solutions via Brokers';
$lang['text_corporate_line1'] = 'AGS Forsikring offers tailored insurance participations for corporate clients through brokers. Our focus is on providing excess capacity and supporting reputable lead insurers, particularly where captive retentions are significant.
<br>
<h2>Key Sectors & Opportunities:</h2>
<br>
<strong>Property / Casualty: </strong>
<br>
- Hydro Power, National Grid, Food & Dairy, Waste Management ? opportunities for excess placements above captive layers and behind leading carriers.
<br>
<strong>Construction:</strong>
<br>
- Large-scale infrastructure projects including metro tunnels, water treatment plants, and office developments, as well as emerging hydrogen factory sites.
<br>

<strong>Risk Appetite:</strong>
<br>

- AGS Forsikring has a broad appetite across industries including molten metal, extrusion, fish, food, pulp & paper, offshore supply, subsea cables, renewable energy (wind, hydro), infrastructure, and railways.
<br>

We are committed to partnering with brokers to support complex risks and provide flexible excess insurance solutions.';



//------------------------cookies
$lang['text_cookies_para'] = "Vi bruger cookies for at sikre korrekt funktionalitet, indsamle analyser og understøtte markedsføringsaktiviteter. 
                        <br>Ved at klikke på <strong>“Accepter alle”</strong> accepterer du brugen af disse teknologier til alle formål. 
                        <br>Alternativt kan du tilpasse dine præferencer ved at vælge de relevante afkrydsningsfelter og klikke på <strong>“Gem indstillinger”</strong>.";
$lang['text_cookies_accept'] = "Accepter alle";
$lang['text_cookies_settings'] = "Cookieindstillinger";
$lang['text_cookies_save'] = "Gem indstillinger";
$lang['text_cookies_functional'] = "Funktionelle cookies";
$lang['text_cookies_analytics'] = "Analysecookies";
$lang['text_cookies_marketing'] = "Markedsføringscookies";


//-------------- Federations and Associations
$lang['text_forbund_title'] = 'Forbund og Foreninger';
$lang['text_forbund_info'] = 'Kontakt os:';
$lang['text_forbund_sub_heading'] = 'Din tryghed er vores erfaring';
$lang['text_forbund_line1'] = 'AGS Forsikring AS leverer skræddersyede forsikringsløsninger til forbund og foreninger. Med mange års erfaring og solide referencer giver vi tryghed gennem professionelle og tilpassede ordninger. Vi benytter kun forsikringsselskaber med top internationale ratings, så I kan være sikre på den højeste kvalitet. Hos os møder I kompetence, forudsigelighed og personlig opfølgning – altid med kundens bedste i fokus.';
$lang['text_daglig'] = 'Daglig Leder';