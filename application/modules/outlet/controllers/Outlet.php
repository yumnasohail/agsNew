...
class Outlet extends MX_Controller {
    ...
    function create() {
        ...
        ///////////////////////////umar apis start/////////////////////////
        $data['restaurant_type'] = Modules::run('api/get_specific_table_with_pagination_where_groupby',array(), 'id desc','id','outlet_types','id,type,delete_status,status','1','0','','','')->result_array();
        ///////////////////////////end umar apis/////////////////////////
    ...
    function set_outlet_product_timing_all() {
        $outlet_id = $this->input->post('outlet_id');
        if(!empty($outlet_id)) {
            Modules::run('product/check_specific_table',$outlet_id,'product_timing');
            $products = Modules::run('api/get_specific_table_data',array(),'id asc','id',$outlet_id.'_products','1','0')->result_array();
            if(!empty($products)) {
                foreach ($products as $key => $pro):
                    $sunday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Sunday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($sunday[0]['opening']) && !empty($sunday[0]['opening'])) $opening=$sunday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '23:59:59'; if(isset($sunday[0]['closing']) && !empty($sunday[0]['closing'])) $close=$sunday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($sunday[0]['is_closed']) && !empty($sunday[0]['is_closed'])) $is_closed=$sunday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Sunday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Sunday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');

                    $Monday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Monday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($Monday[0]['opening']) && !empty($Monday[0]['opening'])) $opening=$Monday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '23:59:59'; if(isset($Monday[0]['closing']) && !empty($Monday[0]['closing'])) $close=$Monday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($Monday[0]['is_closed']) && !empty($Monday[0]['is_closed'])) $is_closed=$Monday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Monday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Monday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');


                    $Tuesday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Tuesday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($Tuesday[0]['opening']) && !empty($Tuesday[0]['opening'])) $opening=$Tuesday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '23:59:59'; if(isset($Tuesday[0]['closing']) && !empty($Tuesday[0]['closing'])) $close=$Tuesday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($Tuesday[0]['is_closed']) && !empty($Tuesday[0]['is_closed'])) $is_closed=$Tuesday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Tuesday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Tuesday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');


                    $Wednesday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Wednesday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($Wednesday[0]['opening']) && !empty($Wednesday[0]['opening'])) $opening=$Wednesday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '23:59:59'; if(isset($Wednesday[0]['closing']) && !empty($Wednesday[0]['closing'])) $close=$Wednesday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($Wednesday[0]['is_closed']) && !empty($Wednesday[0]['is_closed'])) $is_closed=$Wednesday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Wednesday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Wednesday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');


                    $Thursday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Thursday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($Thursday[0]['opening']) && !empty($Thursday[0]['opening'])) $opening=$Thursday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '23:59:59'; if(isset($Thursday[0]['closing']) && !empty($Thursday[0]['closing'])) $close=$Thursday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($Thursday[0]['is_closed']) && !empty($Thursday[0]['is_closed'])) $is_closed=$Thursday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Thursday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Thursday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');

                    $Friday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Friday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($Friday[0]['opening']) && !empty($Friday[0]['opening'])) $opening=$Friday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '23:59:59'; if(isset($Friday[0]['closing']) && !empty($Friday[0]['closing'])) $close=$Friday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($Friday[0]['is_closed']) && !empty($Friday[0]['is_closed'])) $is_closed=$Friday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Friday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Friday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');

                    $Saturday = Modules::run('api/_get_specific_table_with_pagination',array("day" =>'Saturday',"outlet_id" =>$outlet_id), 'id desc','timing','opening,closing,is_closed','1','1')->result_array();
                    $opening = '00:00:00'; if(isset($Saturday[0]['opening']) && !empty($Saturday[0]['opening'])) $opening=$Saturday[0]['opening']; $name=  Modules::run('api/string_length',$opening,'8000','');
                    $close = '00:00:00'; if(isset($Saturday[0]['closing']) && !empty($Saturday[0]['closing'])) $close=$Saturday[0]['closing']; $name=  Modules::run('api/string_length',$close,'8000','');
                    $is_closed = '0'; if(isset($Saturday[0]['is_closed']) && !empty($Saturday[0]['is_closed'])) $is_closed=$Saturday[0]['is_closed']; $name=  Modules::run('api/string_length',$is_closed,'8000','');
                    $query =Modules::run('api/insert_or_update',array("pt_product_id"=>$pro['id'],"pt_day"=>'Saturday'),array("pt_product_id"=>$pro['id'],"pt_day"=>'Saturday','pt_opening'=>$opening,"pt_closing"=>$close,"pt_outlet_id"=>$outlet_id,'pt_is_closed'=>$is_closed),$outlet_id.'_product_timing');
                endforeach;
            }
        }
    }
    ...
    function submit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (is_numeric($update_id) && $update_id != 0) {
                ...
                $old_data['outlet_cover_image'] = $outlet_cover_image;
                ///////////////////////////umar apis start/////////////////////////
    ...
    function _update_theme_data($id, $data, $old_data) {
        ...
        if( ($is_confirm == 'yes')){
            ...
            // $this->_recurse_copy(THEMES_BASE_URL . $template_name . "/images", THEMES_BASE_URL . $name . "/images");
             ...
             if(!file_exists(THEMES_BASE_URL . $name))
              mkdir(THEMES_BASE_URL . $name);
             if(!file_exists(THEMES_BASE_URL . $name.'/css'))
              mkdir(THEMES_BASE_URL . $name.'/css');
            copy(THEMES_BASE_URL . $template_name . "/css/" . $template_name . ".css", THEMES_BASE_URL . $name . '/css/' . $template_name . ".css");
            $this->_recurse_copy(THEMES_BASE_URL . $template_name . "/images", THEMES_BASE_URL . $name . "/images");
        }
  ...
  /*  function _handle_logo($update_id, $itemInfo) {
    ...
    function _get_data_from_db($update_id) {
        ...
        foreach ($query->result() as $row) {
            ...
            $data['restaurant_type'] = $row->restaurant_type;
    ...
    function _get_data_from_post() {
        ...
        $data['restaurant_type'] = $this->input->post('restaurant_type');
        return $data;
    ...
    function delete() {
        ...
        $this->delete_dietary(array("od_outlet_id"=>$id),'outlet_dietary');
...
//        $this->_delete_dir(THEMES_BASE_URL . $name);
    ...
    function set_outlet_password() {
        $data['update_id'] = $update_id = $this->input->post('id');
    ...
    function change_password(){
        ...
        if(!empty($id) && !empty($password))
            Modules::run('api/insert_or_update',array("id"=>$id),array("password"=>$password),'outlet');
    }
    ...
    function delete_table($id){
        $this->load->model('mdl_outlet');
        $this->mdl_outlet->delete_table($id);
    }
    ...
    function check_duplicate_email() {
        ...
        if(!empty($id) && !empty($email)) {
            $checking_name = Modules::run('api/_get_specific_table_with_pagination',array("REPLACE(lower(email), ' ', '' ) =" =>$email,"id !="=>$id),'id desc','outlet','name','1','0')->num_rows();
            if($checking_name == 0)
                $data['response'] = true;
        }
        header('Content-Type: application/json');
