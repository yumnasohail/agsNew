      ...
      <div class="col-md-12">
        <div class="content-wrapper">
          <h3>
            ...
            <a href="javascript:void(0);"><button type="button" style="margin-right: 20px;" class="btn btn-primary pull-right product_timing">Set product timing</button></a>
          </h3>
        </div>
      </div>
    </div>
...
$(document).off("click", ".time_update").on("click", ".time_update", function (event) {
    ...
    $(document).off("click", ".product_timing").on("click", ".product_timing", function () {
        $('.product_timing').html('<image src="https://ui-ex.com/images/transparent-blue-loading-image-gif-1.gif" width="18px">')
        $.ajax({
            type: 'POST',
            url: "<?= ADMIN_BASE_URL ?>outlet/set_outlet_product_timing_all",
            data: {'outlet_id':'<?php echo DEFAULT_OUTLET;?>'},
            success: function (result) {
                $('.product_timing').html('Set product timing')
            }
        });
    });
</script>
