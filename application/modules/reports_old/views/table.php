...
<table class="data-table data-table-feature table table-bordered border-theme1" id="tblReportData" >
                                ...
                                <td><?php  $res=Modules::run('api/_get_specific_table_with_pagination',array("transaction.trans_status"=>"transferred","transaction.claim_id"=>$value['id']), "id desc","transaction","SUM(transaction.belop) as paid_amount",1,0)->row_array();
                                if(empty($res['paid_amount']))
                                 $res['paid_amount']="0.00";
                                echo $res['paid_amount']; ?>
                                </td>
                                <td><?php echo $res['reserve_amount']+$res['paid_amount']; ?></td>
                                <td>
