        ...
        switch($formdata['report'])
        {
            ...
            case "8":
                ...
                if(!empty($formdata['insurer']) && isset($formdata['insurer'])){
                    ...
                    //  $where.="`premiums`.`dato` <= '$c_end' AND ";
                $where= preg_replace('/\W\w+\s*(\W*)$/', '$1', $where);
                $result=$this->get_data_of_reports_bdx($where, "policy_period.start_date desc","premiums.period_id","premiums","premiums.period_id as period_id,SUM( CASE WHEN premiums.dato <= '$c_end'  THEN premiums.paid END) as paid,SUM( CASE WHEN premiums.dato BETWEEN '$c_start' AND '$c_end'  THEN premiums.paid END) as c_paid, SUM( CASE WHEN premiums.dato BETWEEN '$c_start' AND '$c_end'  THEN premiums.recieved END) as recieved ,premiums.comission,SUM( CASE WHEN premiums.dato BETWEEN '$c_start' AND '$c_end'  THEN premiums.total_insurances END) as total_insurances,policy_period.contract_id,policy_period.ags_policy_no,policy_period.currency,policy_period.deductible,policy_period.claim_fee,policy_period.start_date,policy_period.end_date,policy_period.insured_amt,insurers.name,federations.name as f_name,federations.id as f_id,policy_period.policy_id as p_id,policies.name as p_name","","","","","")->result_array();
