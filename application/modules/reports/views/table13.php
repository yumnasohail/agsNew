...
<table class="  table table-bordered border-theme1 table-responsive" id="tblReportData" >
                                ...
                                <tr class="odd gradeX " >
                                        ...
                                        <td><?php if($value['paid_new']>0) echo number_format(round($value['paid_new']), 0, ',', ''); else echo "Nil"; ?></td>
                                        <td>
                                        ...
                                        <?php 
                                        $lr=0;
                                        $ttl=$value['reserve_r']+$value['paid_p'];
                                         if($ttl>0 && $value['paid']>0 ) { $lr=round((round($ttl)/round($value['paid']))*100);}
                                        
                                        $total_sumout=ceil($value['recieved_rv'])+ceil($value['sum_claim_fee'])+ceil($value['paid']/100*10)+ceil($value['paid_p'])+ceil($value['reserve_r']);
                                        $PC_basis=round($value['paid'])-round($total_sumout);
                                        $pc_per_client=0;
                                        if($PC_basis>0){
                                            if($lr<50){
